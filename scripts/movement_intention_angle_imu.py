...
class Calibracion(object):
    def __init__(self):
        ...
        rospy.Subscriber('/imu_data', IMUData, self.updateIMUDataNonParetic)
    ...
    def juego_online(self):
        ...
        if self.isIMUNonPareticUpdated == True:
            self.mi_socket.send(str(self.nonparetic_data).encode("ascii"))
            #self.mi_socket.send(bytes(str(self.nonparetic_data)))
            rospy.loginfo(self.nonparetic_data)
            """
