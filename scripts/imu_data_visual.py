...
class IMU_BNO055:
    ...
    def __init__(self, bus = 3, address = 0x29):
        ...
        self.pub_raw = rospy.Publisher("/imu_raw", Imu, queue_size = 1, latch = False)
    ...
    def read_calib(self):
        ...
        """
        stat = self.read_bytes(self.BNO055_CALIB_STAT, 1)
    ...
    def self_test(self):
        ...
        """
        result = self.read_bytes(self.BNO055_ST_RESULT, 1)
...
def main():
	...
	raw_data.header.stamp = rospy.Time.now()
	raw_data.orientation.x = sensor.quat['x']
	raw_data.orientation.y = sensor.quat['y']
	raw_data.orientation.z = sensor.quat['z']
	raw_data.orientation.w = sensor.quat['w']
	raw_data.orientation_covariance[0] = -1
	raw_data.linear_acceleration.x = sensor.accel['x']
	raw_data.linear_acceleration.y = sensor.accel['y']
	raw_data.linear_acceleration.z = sensor.accel['z']
	raw_data.linear_acceleration_covariance[0] = -1
	raw_data.angular_velocity.x = sensor.gyro['x']
	raw_data.angular_velocity.y = sensor.gyro['y']
	raw_data.angular_velocity.z = sensor.gyro['z']
	raw_data.angular_velocity_covariance[0] = -1
        sensor.pub_raw.publish(raw_data)
